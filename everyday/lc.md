...
前缀树的基础使用，注意细节 ，避免爆NPE。

1. 每个节点初始化时，内部`Node`数组就要初始化好，且长度为26（使用map也可）
2. 迭代添加，注意最后一个节点的`isEnd`属性置为true
3. 递归搜索，需要明确递归函数的返回结果。以及内部的终止条件
    ...
    private boolean search(String word, Node root, int idx) {
        ...
        if (ch == '.') {
            for (Node node : root.next) {
                if (search(word, node, idx + 1)) {
                    return true;
                }
            }
            return false;
        }
        return search(word, root.next[ch - 'a'], idx + 1);
